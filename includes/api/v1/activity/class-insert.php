  	...
  	class SP_Insert_Activity {
        ...
        public static function insert_activty(){
            ...
            $wpdb->query("START TRANSACTION");
                ...
                $wpdb->query("INSERT INTO $table_activity $table_activity_fields VALUES ('$wpid', '$stid', '{$user["activity_icon"]}', '$title_last_id', '$info_last_id',  null, '$date' ) ");
                $activity_last_id = $wpdb->insert_id;

                $wpdb->query("UPDATE $table_activity SET hash_id = sha2($activity_last_id, 256) WHERE ID = $activity_last_id");

                $update_parent_id = $wpdb->query("UPDATE $table_revision SET `parent_id` = $activity_last_id, hash_id = sha2($info_last_id, 256) WHERE ID = $info_last_id");
                $wpdb->query("UPDATE $table_revision SET `parent_id` = $activity_last_id, `hash_id` = sha2($title_last_id, 256) WHERE ID = $title_last_id");
